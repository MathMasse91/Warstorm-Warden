...
local CHAPTERS = {
    ...
    },
    {
        ...
        body = {
            ...
            { kind = "table", rows = {
                ...
                { "/wm",      "Toggle WardenMantle (bot-management HUD)." },
    ...
    },
    {
        ...
        body = {
            { kind = "lede", text = "A floating bot-management card. One place to Summon, Autogear, and ResetBot \226\128\148 the bot-init re-gear by rarity tier \226\128\148 built on Warden's own engine." },
            { kind = "h2",   text = "Open and close" },
            ...
            { kind = "h2",   text = "Lock and unlock" },
            { kind = "p",    text = "The |kbd[o]| circle on the header toggles drag-lock, exactly like WardenSword." },
            { kind = "h2",   text = "Target line" },
            { kind = "p",    text = "Shows the current target's portrait, class-colored name, and level. With no valid player target the card shows |em[No Target]| \226\128\148 the actions below are global, so they still work." },
            { kind = "h2",   text = "Actions" },
            { kind = "p",    text = "|ck[Summon]| recalls bots, |ck[Autogear]| sends |ck[autogear]| to PARTY. Pick a gear tier in the rarity dropdown, then |ck[ResetBot]| re-rolls every bot via |ck[.warstormbot bot init=<rarity>]| on the command channel \226\128\148 the same \"level up\" logic the Spec tab exposes." },
            ...
            { kind = "table", rows = {
                { "/wm",                "Toggle the HUD." },
                { "/wm show / hide",    "Explicit visibility." },
                { "/wm lock / unlock",  "Drag-lock toggle." },
                { "/wm reset",          "Reset HUD position." },
            }},
        },
    },
    {
    ...
    },
    {
        ...
        body = {
            ...
            { kind = "table", rows = {
                ...
                { "/wm",            "Toggle the WardenMantle bot-management HUD." },
                { "/wm lock / unlock / reset", "Drag-lock toggle / reset position." },
                { "/wardenmantle",  "Long alias for |ck[/wm]|." },
